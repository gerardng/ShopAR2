...
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    ...
    void initRendering()
    {
        ...
        keyframeQuadAspectRatio[VideoPlayback.IMAGE_TARGET_1] = (float) mTextures
            .get(0).mHeight / (float) mTextures.get(0).mWidth;
        keyframeQuadAspectRatio[VideoPlayback.IMAGE_TARGET_2] = (float) mTextures.get(1).mHeight
    ...
    public void renderFrame(State state, float[] projectionMatrix)
    {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
        {
            ...
            if (imageTarget.getName().compareTo("stones") == 0)
                currentTarget = VideoPlayback.IMAGE_TARGET_1;
            else
                currentTarget = VideoPlayback.IMAGE_TARGET_2;
        ...
        float[] textureCoordMatrix)
    {
        ...
        if (target == VideoPlayback.IMAGE_TARGET_1)
        {
        ...
        } else if (target == VideoPlayback.IMAGE_TARGET_2)
        {
            tempUVMultRes = uvMultMat4f(
